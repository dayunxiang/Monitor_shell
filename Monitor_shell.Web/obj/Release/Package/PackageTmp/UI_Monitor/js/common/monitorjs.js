...
function GetDcsTime()
{
    ...
    $('#ConnectionGround').css('display', 'none');
    var urlString = "/UI_Monitor/DCSMonitor/MonitorShell/DCSMonitorShell.asmx/GetDcsTime"
    $.ajax({
        type: "POST",
        url: urlString,
        data: JSON.stringify(dataToServer),
...
{
    $('#ConnectionGround').css('display', 'none');
    var textNode = $("#TextGround");
    document.getElementById("Picture").style.backgroundImage = "url(/UI_Monitor/images/common/signalErr.jpg)";
    var colorControl = document.getElementById("TextGround");
    colorControl.style.color = 'yellow';//控制文字颜色
    textNode.text("");//清空文字
    textNode.append("未能获取DCS时间");//控制文字内容
    textNode.append(DcsTime);
}
function textInnation(dcsTime) {
    for(var x in dcsTime){
    }
    var DcsTime = dcsTime[x];
    var dt = new Date(DcsTime.replace(/-/g, "/"));
    ...
    if (Intmin > 20) {
        ...
        oBox.onmouseover = function () { //当鼠标移入div时 闪烁层消失
            clearInterval(timer);
            $('#ConnectionGround').css('display', 'none');
        };
        timer = setInterval(function () {
            oBox.style.display = i++ % 2 ? "none" : "block";  // 有规律的控制div的展示与隐藏  
            i > 1 && (clearInterval(timer))  //控制闪烁次数  
        }, 450);
        var textNode = $("#TextGround");
        document.getElementById("Picture").style.backgroundImage = "url(/UI_Monitor/images/common/signalErr.jpg)";
        var colorControl = document.getElementById("TextGround");
    ...
    else
    {
        $('#ConnectionGround').css('display', 'none');
        var textNode = $("#TextGround");
        document.getElementById("Picture").style.backgroundImage = "url(/UI_Monitor/images/common/signalCorrect.jpg)";
        var colorControl = document.getElementById("TextGround");
        colorControl.style.color = '#4cff00';//控制文字颜色
        textNode.text("");//清空文字
        textNode.append("数据时间: ");//控制文字内容
        textNode.append(DcsTime);
    }
...
function GaugesInfosService() {
    var organizationIdName = publicData.organizationId;
    var sendData = {
        organizationId: publicData.organizationId,
        tagSet:publicData.ids
           // AnlogSignalTags
    };
    var sendDataNew = {
        organizationId: publicData.organizationId,
...
function DealWidthAlarmColor(gaugeObj) {
    ...
    var organiazationName = gaugeObj.OrganiazationName
    var resultTag = organiazationName + ">" + dcsTag + ">DCS";
