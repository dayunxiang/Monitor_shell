...
namespace Monitor_shell.Service.ProcessEnergyMonitor.EnergyContrast
{
    public class EnergyContrastHelper
    {
        ...
        public IEnumerable<DataItem> GetEnvironmentDatas(DateTime myTime,string organizationId, IList<string> variableNames)
        {
            ...
            DataTable table = GetEnvironmentTable(myTime, fieldInformations);
            string[] idList = GetTableColumnName(table);
            foreach (var item in idList)
            {
                string t_value = "0";
                ...
                else
                {
                    try
                    {
                        t_value = table.Rows[0][item] is DBNull ? "0" : Convert.ToDecimal(table.Rows[0][item]).ToString("0.00").Trim();
                    }
                    catch
                    {
                        t_value = "";
        ...
        private DataTable GetEnvironmentTable(DateTime myTime, IDictionary<string, List<FieldInformation>> fieldInformations)
        {
            DataTable result = new DataTable();
            if (fieldInformations.Keys.Count() > 0)
            {
                string queryString = GetEnvironmentSqlString (myTime, fieldInformations);
                result = _dataFactory.Query(queryString);
            }
            return result;
        ...
        //获取环境语句
        private static string GetEnvironmentSqlString(DateTime myTime, IDictionary<string, List<FieldInformation>> feildInfor)
        {
            StringBuilder result = new StringBuilder();
            result.Append("select ");
            StringBuilder feilds = new StringBuilder();
            ...
            for (int i = 0; i < feildInfor.Count; i++)
            {
                ...
                string key = keyArray[i];
                tables.Append(key + " as " + key_new + ",");
                foreach (var feild in feildInfor[key])
                {

                    feilds.Append("AVG(");
                    string tempFeild = key_new + ".[" + feild.FeildName + "]";
                    feilds.Append(tempFeild);
                    feilds.Append(")");
                    feilds.Append("  as " + feild.VariableName + "");
                    feilds.Append(",");
                }
                if (i == 0)
                {
                    criteria.Append(key_new + ".vDate<'" + myTime.ToString() + "'");
                    criteria.Append(" and ");
                    criteria.Append(key_new + ".vDate>='" + myTime.AddHours(-1).ToString() + "'");
                    criteria.Append(" and ");
                }
                else
                {
                    criteria.Append(key_new + ".vDate = " + "A" + ".vDate ");
                    criteria.Append(" and ");
                }
            }
            criteria.Remove(criteria.Length - 5, 5);
